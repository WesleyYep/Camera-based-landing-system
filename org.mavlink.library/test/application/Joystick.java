...
public class Joystick extends JFrame {
    ...
    public Joystick(NetworkConnection connection) {
        setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
        setSize(displayWidth, displayHeight);
        setLocationRelativeTo(null);
        position = new Point();
        SimpleJoystick myJoystick = new SimpleJoystick(100, position, 100, connection);
...
class SimpleJoystick extends JPanel {
    ...
    public SimpleJoystick(final int joyOutputRange,
            final Point position, final int joySize, NetworkConnection connection) {
    ...
    private void mouseCheck(final MouseEvent e) {
        ...
        position.y = (int) (joyOutputRange * (-(Math.sin(curJoyAngle)
                * curJoySize) / joySize));
        try {
			connection.send("command:" + position.x + ":" + position.y);
		} catch (Exception e1) {
			e1.printStackTrace();
		}
